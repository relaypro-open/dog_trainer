...
-spec wait_on_rethinkdb(Connection :: pid(), DatabaseName :: binary()) -> {ok, any()}.
wait_on_rethinkdb(Connection, DatabaseName) ->
    {ok, _Databases} = gen_rethink:run(
        Connection,
        fun(X) ->
...
-spec ensure_table_exists(Connection :: pid(), DatabaseName :: binary(), TableName :: string()) ->
    ok | change.
ensure_table_exists(Connection, DatabaseName, TableName) ->
    {ok, Tables} = gen_rethink:run(
        Connection,
        fun(X) ->
            reql:db(X, DatabaseName),
            reql:table_list(X)
...
-spec create_table(Connection :: pid(), DatabaseName :: binary(), TableName :: string()) ->
    {ok, map()}.
create_table(Connection, DatabaseName, TableName) ->
    {ok, #{}} = gen_rethink:run(
        Connection,
...
-spec create_index(Connection :: pid(), TableName :: string(), FieldName :: string()) ->
    {ok, map()}.
create_index(Connection, TableName, FieldName) ->
    gen_rethink:run(
        Connection,
