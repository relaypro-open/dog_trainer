...
handle_connection_up(Connection, State) ->
    ...
    ?LOG_DEBUG(#{message => "handle_connection_up"}, #{domain => [dog_trainer]}),
    ?LOG_DEBUG(#{connection => Connection}, #{domain => [dog_trainer]}),
    Reql = reql:db(<<"dog">>),
...
handle_connection_down(State) ->
    ?LOG_DEBUG(#{message => "handle_connection_down"}, #{domain => [dog_trainer]}),
    {noreply, State}.
