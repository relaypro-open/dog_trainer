...
new_name(NameBase, Type, Version) ->
    ...
    NewSuffix = <<"n">>,
    NewName =
        <<NameBase/bitstring, Sep/bitstring, Type/bitstring, Version/bitstring,
            NewSuffix/bitstring>>,
    NewName.
...
-spec create_ipset(Name :: binary(), NewName :: binary(), Ips :: list(), Version :: binary()) ->
    list().
create_ipset(Name, NewName, Ips, Version) ->
    Inet =
        case Version of
...
add_to_ipset(Name, Ip) ->
    ?LOGT_DEBUG("add_to_ipset(Name,Ip): ~p, ~p", [{name,Name}, {ip,Ip}]),
...
get_hashes() ->
    ...
    ValidHashes = [
        maps:get(<<"hash">>, Hash)
     || Hash <- ValidIpsets, maps:get(<<"hash">>, Hash) =/= <<"initial">>
    ],
...
id_maps() ->
    {
        {ExternalUnionGroupIpv4sGrouped, ExternalUnionGroupIpv6sGrouped,
            ExternalUnionZoneIpv4sGrouped, ExternalUnionZoneIpv6sGrouped},
        {ExternalPrefixGroupIpv4sGrouped, ExternalPrefixGroupIpv6sGrouped,
            ExternalPrefixZoneIpv4sGrouped, ExternalPrefixZoneIpv6sGrouped}
    } = dog_external:grouped_by_ipset_name(),
    ...
    ServiceAny =
        case dog_service:get_by_id(<<"any">>) of
            {ok, Service} ->
                Service;
            {error, _} ->
...
create_merged_ipsets_list(MergedGroups) ->
    {GroupIpv4sGrouped, GroupIpv6sGrouped, ZoneIpv4sGrouped, ZoneIpv6sGrouped} = MergedGroups,
    Ipv4s = #{
        <<"zones">> => ZoneIpv4sGrouped,
        <<"groups">> => GroupIpv4sGrouped
    },
    CreateIpsets4 = create_ipsets(Ipv4s, <<"v4">>),
    Ipv6s = #{
        <<"zones">> => ZoneIpv6sGrouped,
    ...
    Ipsets = [
        CreateIpsets4, CreateIpsets6, FlushIpsets4, FlushIpsets6, DestroyIpsets4, DestroyIpsets6
    ],
    Ipsets.
...
merge_ipsets(
    {GroupIpv4sGrouped1, GroupIpv6sGrouped1, ZoneIpv4sGrouped1, ZoneIpv6sGrouped1},
    {GroupIpv4sGrouped2, GroupIpv6sGrouped2, ZoneIpv4sGrouped2, ZoneIpv6sGrouped2}
) ->
...
-spec create_internal_groups() ->
    {
        InternalGroupIpv4sGrouped :: map(),
        InternalGroupIpv6sGrouped :: map(),
        InternalZoneIpv4sGrouped :: map(),
        InternalZoneIpv6sGrouped :: map()
    }.
...
create_internal_ipsets(
    {InternalGroupIpv4sGrouped, InternalGroupIpv6sGrouped, InternalZoneIpv4sGrouped,
        InternalZoneIpv6sGrouped}
) ->
    InternalIpv4s = #{
        <<"groups">> => InternalGroupIpv4sGrouped,
        <<"zones">> => InternalZoneIpv4sGrouped
    },
    InternalIpv6s = #{
        <<"groups">> => InternalGroupIpv6sGrouped,
        <<"zones">> => InternalZoneIpv6sGrouped
    },
    InternalIpsetsMap = #{
        <<"name">> => binary:list_to_bin(application:get_env(dog_trainer, env, "NOTSET")),
        <<"v4">> => InternalIpv4s,
        <<"v6">> => InternalIpv6s
    },
    InternalIpsetsMap.
...
publish_to_queue(Ipsets) ->
    ...
    ?LOGT_DEBUG("Ipsets: ~p", [{ipsets,Ipsets}]),
    UserData = #{
...
publish_to_outbound_exchanges(IpsetExternalMap) ->
    ...
    lists:foreach(
        fun(Env) ->
            ...
            ?LOGT_DEBUG("ExternalMap: ~p~n", [{external_map,ExternalMap}]),
...
publish_to_outbound_exchange(TargetEnvName, IpsetExternalMap) ->
    ?LOGT_INFO("IpsetExternalMap: ~p", [{ipset_external_map,IpsetExternalMap}]),
...
hash_check(AgentIpsetHash) ->
    ...
    case AgentIpsetHash == LatestHash of
        false ->
            ?LOGT_INFO("Host IpsetHash ~p not equal to Latest IpsetHashes: ~p", [
...
update_ipsets(Env) ->
    ...
    delete_old(),
    create(NewIpsetHash),
    ?LOGT_DEBUG("LastestHash, NewIpsetHash: ~p, ~p", [{latest_hash,LatestHash}, {new_ipset_hash,NewIpsetHash}]),
...
persist_ipset() ->
    ...
    case os:cmd(PersistCmd) of
        [] ->
            ok;
        Result ->
            ?LOGT_ERROR("PersistCmd Error: ~p", [{result,Result}]),
            {error, Result}
    end.
...
read_hash() ->
    NormalizedIpset = normalize_ipset(read_current_ipset()),
    IpsetHash = create_hash(NormalizedIpset),
    ?LOGT_INFO("ipset hash: ~p", [{ipset_hash,IpsetHash}]),
    IpsetHash.
