...
send_hash_alert(Id) ->
    ...
    {ok,Addresses} = application:get_env(dog_trainer,smtp_to),
    To = string:join(Addresses,","),
    Body = io_lib:format("Host's iptables and/or ipsets modified outside of dog~nHostName: ~s~nHostKey: ~s~nHost: ~p~nGroup: ~p~n",[HostName, HostKey,Host,Group]),
    Email = io_lib:format("Subject: ~s\r\nFrom: ~s \r\nTo: ~s \r\n\r\n~s",[Subject,From,To,Body]),
    gen_smtp_client:send({From, Addresses, Email},
                       [{relay, SmtpRelay}, {username, SmtpUsername}, {password, SmtpPassword},
                        {tls,always}]),
...
send_hash_recover(Id) ->
    ...
    {ok,Addresses} = application:get_env(dog_trainer,smtp_to),
    To = string:join(Addresses,","),
    Body = io_lib:format("Host's iptables and/or ipsets back in sync with dog: ~nHostName: ~s~nHostKey: ~s~nHost: ~p~nGroup: ~p~n", [HostName,HostKey,Host,Group]),
    Email = io_lib:format("Subject: ~s\r\nFrom: ~s \r\nTo: ~s \r\n\r\n~s",[Subject,From,To,Body]),
    gen_smtp_client:send({From, Addresses, Email},
                       [{relay, SmtpRelay}, {username, SmtpUsername}, {password, SmtpPassword},
                        {tls,always}]),
