...
-export([
    ...
    read_iptables_ruleset_unset_v6_from_file/1,
    write_iptables_ruleset_set_v4_to_file/2,
    write_iptables_ruleset_set_v6_to_file/2,
    write_iptables_ruleset_unset_v4_to_file/2,
...
generate_iptables_ruleset(
    ...
    try
        ...
        InboundRules = lists:map(
            ...
            InboundJsonWithIndex
        ),
        %Non-Basic rules are put in INPUT only
        InboundJsonBasicOnly = lists:filter(
            fun(Rule) ->
                maps:get(<<"type">>, Rule) == <<"BASIC">>
            end,
            InboundJson
        ),
        InboundJsonBasicOnlyWithIndex = lists:zip(
            lists:seq(1, length(InboundJsonBasicOnly)), InboundJsonBasicOnly
        ),
        InboundSymmetricRules = lists:map(
            fun({Index, L}) ->
        ...
        imetrics:add_m(generate_iptables_ruleset, "ok"),
        {ok, IptablesRuleset}
    catch
        Exception:ExceptionReason:Stacktrace ->
    ...
    case Json of
        ...
        _ ->
            case get_service(maps:get(<<"service">>, Json)) of
                ...
                %not sure if we want to create some better error handling here or just return empty iptables_ruleset
                <<" ">> ->
                    [];
                ServiceId ->
                            ...
                            ipsets ->
                                case RuleType of
                                    ...
                                    connlimit ->
                                        ...
                                        ConnLimitString = get_connlimit(
                                            ConnLimitAbove, ConnLimitMask, Version
                                        ),
                                        generate_connlimit_rule_set(
                                            Chain,
                                            Interface,
                                            ProtocolString,
                                            PortParameter,
                                    ...
                                    recent ->
                                        RecentName = maps:get(
                                            <<"recent_name">>, Json, <<"DEFAULT">>
                                        ),
                                        RecentMask = maps:get(<<"recent_mask">>, Json, []),
                                        Seconds = maps:get(<<"seconds">>, Json),
                                        HitCount = maps:get(<<"hit_count">>, Json),
                                        RecentString = get_recent(
                                            RecentName, RecentMask, Seconds, HitCount, Version
                                        ),
                                        RecentRule = generate_recent_rule_set(
                                            Chain,
                                            Interface,
                                            ProtocolString,
                                            PortParameter,
...
get_recent(RecentName, RecentMask, Seconds, HitCount, Version) ->
    case RecentMask of
        [] ->
            ...
            io_lib:format(" -m recent --update --seconds ~B --hitcount ~B --name ~s --mask ~s", [
                Seconds, HitCount, RecentName, DefaultRecentMask
            ]);
        _ ->
            io_lib:format(" -m recent --update --seconds ~B --hitcount ~B --name ~s --mask ~s", [
                Seconds, HitCount, RecentName, RecentMask
            ])
    end.

generate_recent_rule_set_set(
    Chain,
    Interface,
    ProtocolString,
    PortParameter,
...
-spec get_connlimit(ConnLimitAbove :: binary(), ConnLimitMask :: binary(), Version :: binary()) ->
    iolist().
get_connlimit(ConnLimitAbove, ConnLimitMask, Version) ->
    case ConnLimitMask of
        [] ->
            DefaultConnLimitMask =
                case Version of
                    <<"v4">> -> <<"32">>;
                    <<"v6">> -> <<"128">>
                end,
            io_lib:format(
                " -m connlimit --connlimit-above ~B --connlimit-mask ~B --connlimit-saddr", [
                    ConnLimitAbove, DefaultConnLimitMask
                ]
            );
        _ ->
            io_lib:format(
                " -m connlimit --connlimit-above ~B --connlimit-mask ~B --connlimit-saddr", [
                    ConnLimitAbove, ConnLimitMask
                ]
            )
    end.
...
-spec get_ipset_name(
    GroupName :: binary(), GroupType :: binary(), Sep :: binary(), Version :: binary()
) -> binary().
get_ipset_name(GroupName, GroupType, Sep, Version) ->
    LocalName =
        case bin_uppercase(GroupName) of
            <<"ANY">> ->
                <<"ANY">>;
            _ ->
                case GroupType of
                    ...
                    G when G =:= <<"ROLE">> ; G =:= <<"GROUP">> ->
                        <<GroupName/bitstring, Sep/bitstring, <<"g">>/bitstring,
                            Version/bitstring>>;
                    <<"ZONE">> ->
                        <<GroupName/bitstring, Sep/bitstring, <<"z">>/bitstring, Version/bitstring>>
                end
        end,
    LocalName.
    ...
    case RuleType of
        ...
        _ ->
            case bin_uppercase(Action) of
                <<"DROP">> ->
                    string:to_upper(binary_to_list(Action));
                <<"ACCEPT">> ->
                    string:to_upper(binary_to_list(Action));
                <<"REJECT">> ->
                    case Version of
        ...
        _ ->
            case Symmetric of
                false ->
                    S = lists:map(
                        fun(X) ->
                            io_lib:format("~s", [binary_to_list(X)])
                        end,
                        States
                    ),
...
write_iptables_ruleset_set_v4_to_file(IptablesRuleset, GroupName) ->
    FileName = ?RUNDIR ++ "/iptables_ruleset4_ipsets." ++ binary_to_list(GroupName) ++ ".txt",
    ok = file:write_file(FileName, IptablesRuleset).
write_iptables_ruleset_set_v6_to_file(IptablesRuleset, GroupName) ->
    FileName = ?RUNDIR ++ "/iptables_ruleset6_ipsets." ++ binary_to_list(GroupName) ++ ".txt",
    ok = file:write_file(FileName, IptablesRuleset).
write_iptables_ruleset_unset_v4_to_file(IptablesRuleset, GroupName) ->
    FileName = ?RUNDIR ++ "/iptables_ruleset4_iptables." ++ binary_to_list(GroupName) ++ ".txt",
    ok = file:write_file(FileName, IptablesRuleset).
write_iptables_ruleset_unset_v6_to_file(IptablesRuleset, GroupName) ->
