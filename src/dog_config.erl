...
-spec update_host_hashes(
    Hostname :: binary(),
    Hash4Ipsets :: iolist(),
    Hash6Ipsets :: iolist(),
    Hash4Iptables :: iolist(),
    Hash6Iptables :: iolist(),
    IpsetHash :: iolist()
) -> ok.
update_host_hashes(Hostname, Hash4Ipsets, Hash6Ipsets, Hash4Iptables, Hash6Iptables, IpsetHash) ->
    ?LOG_DEBUG(#{hostname => Hostname, hash4_ipsets => Hash4Ipsets, hash6_ipsets => Hash6Ipsets,
                 hash4_iptables => Hash4Iptables, hash6_iptables => Hash6Iptables, ipset_hash => IpsetHash }),
    %{ok, RethinkTimeout} = application:get_env(dog_trainer,rethink_timeout_ms),
    %{ok, Connection} = gen_rethink_session:get_connection(dog_session),
    {ok, _R} = dog_rethink:run(
        fun(X) ->
