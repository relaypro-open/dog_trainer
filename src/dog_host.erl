...
send_hash_alert(Id) ->
    ...
    HostKey = binary:bin_to_list(maps:get(<<"hostkey">>,Host)),
    GroupName = maps:get(<<"group">>,Host),
    {ok,IpsetHash} = dog_ipset:get_hash(),
    {ok,Group} = dog_group:get_by_name(GroupName),
    ...
    {ok,Addresses} = application:get_env(dog_trainer,smtp_to),
    To = string:join(Addresses,","),
    Body = io_lib:format("Host's iptables and/or ipsets modified outside of dog~nHostName: ~s~nHostKey: ~s~nHost: ~p~nGroup: ~p~nIpsetHash: ~p~n",[HostName, HostKey,Host,Group,IpsetHash]),
    Email = io_lib:format("Subject: ~s\r\nFrom: ~s \r\nTo: ~s \r\n\r\n~s",[Subject,From,To,Body]),
    gen_smtp_client:send({From, Addresses, Email},
                       [{relay, SmtpRelay}, {username, SmtpUsername}, {password, SmtpPassword},
                        {tls,always}]),
...
send_hash_recover(Id) ->
    ...
    {ok,Addresses} = application:get_env(dog_trainer,smtp_to),
    To = string:join(Addresses,","),
    Body = io_lib:format("Host's iptables and/or ipsets back in sync with dog: ~nHostName: ~s~nHostKey: ~s~nHost: ~p~nGroup: ~p~nIpsetHash: ~p~n", [HostName,HostKey,Host,Group,IpsetHash]),
    Email = io_lib:format("Subject: ~s\r\nFrom: ~s \r\nTo: ~s \r\n\r\n~s",[Subject,From,To,Body]),
    gen_smtp_client:send({From, Addresses, Email},
                       [{relay, SmtpRelay}, {username, SmtpUsername}, {password, SmtpPassword},
                        {tls,always}]),
