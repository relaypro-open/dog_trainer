...
get_all_ips() ->
    ...
    Ips = lists:map(
        fun(X) -> [maps:get(<<"ipv4_addresses">>, X), maps:get(<<"ipv6_addresses">>, X)] end,
        Interfaces
    ),
...
get_by_id(ZoneId) ->
    case get_document_by_id(ZoneId) of
        {ok, Zone} ->
            {ok, Zone};
        {error, Error} ->
            ?LOG_ERROR(#{"message" => "zone id not found", "zone_id" => ZoneId}),
            {error, Error}
    end.
...
delete(Id) ->
    case in_profile(Id) of
        ...
        {true, Profiles} ->
            ?LOG_INFO(#{"id" => Id, "message" => "zone not deleted - exists in profiles", "profiles" => " Profiles"}),
