...
resource_exists(Req@0, State@0) ->
    case cowboy_req:method(Req@0) of
        <<"POST">> ->
            ...
            ?LOG_INFO(#{body => Body}, #{domain => [dog_trainer]}),
        ...
        {error, notfound} ->
            case Handler:create(Map) of
                ...
                {ok, Id} ->
                    ...
                    ?LOG_INFO(#{uri => Uri}, #{domain => [dog_trainer]}),
...
from_put_json(Req@0, State) ->
    ...
    ?LOG_INFO(#{inplace => InPlace}, #{domain => [dog_trainer]}),
    Response =
        case InPlace of
            #{inplace := <<"True">>} ->
    ...
    case Response of
        {false, Error} when is_atom(Error) ->
            ?LOG_INFO(#{error => Error}, #{domain => [dog_trainer]}),
            Req@2 = cowboy_req:set_resp_body([atom_to_list(Error)], Req@1),
            {false, Req@2, State};
        {false, ObjectId} ->
            ?LOG_INFO(#{objectid => ObjectId}, #{domain => [dog_trainer]}),
                        ...
                        case Sub of
                            ...
                            undefined ->
                                ...
                                case cowboy_req:match_qs([{git_diff, [], plain}], Req) of
                                    #{git_diff := plain} ->
                                        ?LOG_DEBUG(#{message => "here"}, #{domain => [dog_trainer]}),
                                        case dog_profile:get_by_id(Id) of
                                            {ok, Profile_} ->
